...
namespace NaughtyAttributes.Editor
{
    public static class PropertyUtility
    {
        ...
        private const BindingFlags PropertyFlags = FieldFlags | BindingFlags.IgnoreCase;
        ...
        public static bool IsEnabled(SerializedProperty property)
        {
            ...
            if (conditionValues.Count > 0)
            {
                bool enabled = GetConditionsFlag(
                    conditionValues, enableIfAttribute.ConditionOperator,
        ...
        public static bool IsVisible(SerializedProperty property)
        {
            ...
            {
                bool enabled = GetConditionsFlag(
                    conditionValues, showIfAttribute.ConditionOperator,
        ...
        {
            var targetTypes = ReflectionUtility.GetSelfAndBaseTypes(target);

            FieldInfo enumField = ReflectionUtility.GetField(targetTypes, enumName);
            if (enumField != null && enumField.FieldType.IsSubclassOf(typeof(Enum)))
            {
                return (Enum) enumField.GetValue(target);
            }

            PropertyInfo enumProperty = ReflectionUtility.GetProperty(targetTypes, enumName);
            if (enumProperty != null && enumProperty.PropertyType.IsSubclassOf(typeof(Enum)))
            {
                return (Enum) enumProperty.GetValue(target);
            }

            MethodInfo enumMethod = ReflectionUtility.GetMethod(targetTypes, enumName);
        ...
        internal static List<bool> GetConditionValues(object target, string[] conditions)
        {
            ...
            {
                FieldInfo conditionField = ReflectionUtility.GetField(targetTypes, condition);
                ...
                PropertyInfo conditionProperty = ReflectionUtility.GetProperty(targetTypes, condition);
                ...
                MethodInfo conditionMethod = ReflectionUtility.GetMethod(targetTypes, condition);
        ...
        public static object GetTargetObjectWithProperty(SerializedProperty property)
        {
            ...
            for (int i = 0, elementsLength = elements.Length - 1; i < elementsLength; i++)
            {
